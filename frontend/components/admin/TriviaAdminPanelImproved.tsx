...
import { Plus, Edit2, Trash2, Save, X, Brain, Clock, Target, ChevronDown, ChevronRight } from "lucide-react";
...
export function TriviaAdminPanelImproved({ passcode }: TriviaAdminPanelProps) {
  ...
  const handleSaveLevel = async (level: Partial<TriviaLevel>) => {
    try {
      const base = import.meta.env.DEV ? "http://127.0.0.1:4000" : "https://prod-cne-sh82.encr.app";
  ...
  const handleSaveQuestion = async (question: Partial<TriviaQuestion>) => {
    try {
      const base = import.meta.env.DEV ? "http://127.0.0.1:4000" : "https://prod-cne-sh82.encr.app";
