...
function stripUsfmInline(text: string): string {
  ...
  // Remove character style markers like \add, \wj, \em, \bd, etc. Keep inner text.
...
function parseUsfmFile(contents: string): ParsedVerse[] {
  ...
  for (const rawLine of lines) {
    ...
    // Continuation line: append to current verse buffer.
    if (verseNum != null) {
      verseBuf += " " + line;
    }
  }
  ...
  // If we failed to map a book code, include context in error.
  if (usfmBook && !bookId) {
...
async function importTranslationFromDir(translation: TranslationId, dir: string) {
...
async function main() {
  ...
  if (!translation || !dir) {
    console.error(
      "Usage: bun run backend/bible/import_usfm.ts <translation> <dir>\n" +
        "Example: bun run backend/bible/import_usfm.ts kjv ./data/kjv_usfm"
    );
    process.exit(2);
  }
  ...
  const count = await importTranslationFromDir(translation, abs);
