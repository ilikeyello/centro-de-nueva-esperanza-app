...
// Define the service
export default new Service("trivia", {});
  ...
  async ({ level_id }: { level_id?: string }): Promise<TriviaQuestionsResponse> => {
    ...
    return {
      questions: rows.map((row: any) => ({
        ...
        options_en: typeof row.options_en === 'string' ? JSON.parse(row.options_en) : row.options_en,
        options_es: typeof row.options_es === 'string' ? JSON.parse(row.options_es) : row.options_es,
